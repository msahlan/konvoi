...
class PickupapiController extends BaseController {
    ...
    public function index()
    {
        ...
        $page_size = config('jex.api_page_size');
        ...
        $model = $this->model
            ...
            */
            ->where(function($query) use($deliverydate,$until){
                $query->where('pickup_status','=', config('jayon.trans_status_tobepickup') )
                    ->where('status','!=', config('jayon.trans_status_canceled') )
                    ->where('status','!=', config('jayon.trans_status_mobile_delivered') )
