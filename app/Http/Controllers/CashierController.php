...
class CashierController extends AdminController {
    ...
    public function getIndex()
    {
        ...
        $this->crumb->addCrumb('Cashier',url( strtolower($this->controller_name) ));
        ...
        $model = $model
            ->where(function($q){
                $q->where('status',config('jayon.trans_status_mobile_delivered'))
                    ...
                    ->orWhere('status',config('jayon.trans_status_mobile_pickedup'))
                    ->orWhere('status',config('jayon.trans_status_mobile_enroute'))                    //->where('status',config('jayon.trans_status_admin_courierassigned'))
                    //->orWhere('status',config('jayon.trans_status_new'))
                    ->orWhere('status',config('jayon.trans_status_rescheduled'))
                    ->orWhere('status',config('jayon.trans_status_mobile_return'))
                    ->orWhere(function($qp){
                        $qp->where('status',config('jayon.trans_status_new'))
                          ->where('pending_count','>', 0);
                    });
